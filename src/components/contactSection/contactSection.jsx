...
const ContactSection = ({ settings, socials }) => {
  ...
  return (
    <section
  ref={sectionRef}
  className="relative w-full py-16 md:py-24 lg:py-32 overflow-hidden font-lato bg-gray-50"
>
  {/* 1. PARALLAX BACKGROUND */}
  <motion.div 
    style={{ y: backgroundY }}
    className="absolute inset-0 z-0 opacity-10 grayscale" // Reduced opacity for light theme
  >
    <div 
      className="absolute inset-0 bg-cover bg-center scale-110"
      style={{ backgroundImage: `url(${contactImg})` }}
    />
    <div className="absolute inset-0 bg-white/40 backdrop-blur-sm" /> {/* Changed to white overlay */}
  </motion.div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
      ...
      <div className="order-2 lg:order-1 w-full">
        <div className="mb-8 lg:mb-10">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extralight leading-tight text-gray-900 mb-6"> {/* Changed to gray-900 */}
            Ready to start your <br className="hidden md:block" />
            <span className="italic font-serif text-primary">next chapter</span>?
          </h2>
        </div>

        {/* Glassmorphism Contact Cards - Updated for light theme */}
          ...
          {[
            ...
            >
              ...
              <div className="ml-4 md:ml-5 overflow-hidden">
                ...
                <p className="text-sm md:text-base text-gray-800 font-light break-words"> {/* Changed to gray-800 */}
                  {item.val}
                </p>
              </div>
            </motion.div>
        ...
        {/* Socials - Updated for light theme */}
        <div className="flex flex-wrap items-center gap-4 md:gap-6 border-t border-gray-200 pt-8"> {/* Changed border */}
          <div className="flex flex-wrap gap-3 md:gap-4">
            {sortedSocials.map((social) => (
              <motion.a
                ...
                className="w-10 h-10 md:w-11 md:h-11 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-600 hover:text-primary hover:border-primary transition-colors shadow-sm" // Updated colors
              >
      ...
      >
        ...
        <div className="relative z-10 overflow-hidden md:rounded-2xl rounded-md shadow-xl w-full aspect-[4/5] md:aspect-[3/4] group"> {/* Shadow softened */}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent opacity-80" /> {/* Reduced opacity */}
